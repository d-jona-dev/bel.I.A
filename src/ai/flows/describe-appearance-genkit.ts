...
/**
 * @fileOverview Describes a character's appearance or clothing based on an image using Genkit/Gemini.
 */
...
// Schemas are defined here but can be moved to a shared file if needed
...
const describeAppearancePrompt = ai.definePrompt({
  name: 'describeAppearanceGenkitPrompt',
...
export async function describeAppearanceWithGenkit(input: DescribeAppearanceInput): Promise<DescribeAppearanceOutput> {
    try {
        ...
        console.error("Error in describeAppearanceWithGenkit flow:", e);
