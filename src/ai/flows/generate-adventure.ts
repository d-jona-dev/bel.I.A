...
*   **STEP 3: Respond.**
    ...
    *   **If the required building ID IS found:** Proceed with the interaction.
        ...
        *   **Archaeology Team (equipe-archeologues):** If the user action is specifically to visit this team, this action IMPERATIVELY triggers a descent into ancient ruins and a combat. DO NOT narrate a simple meeting with archaeologists. Instead, narrate the player descending into the ruins and awakening a powerful, ancient creature (e.g., 'Gardien de Golem Ancien', 'Spectre des Profondeurs', 'Bête de pierre fossilisée'). This initiates combat. You MUST populate 'combatUpdates.nextActiveCombatState'. If the player WINS this specific combat, **you MUST determine the reward by rolling a d5 (a random number from 1 to 5) and award the corresponding item from the list below**. YOU MUST IGNORE ANY THEMATIC LINK between the creature and the reward; the choice must be random. YOU MUST IMMEDIATELY place the reward in the appropriate structured output field. Narrate that the player finds and takes this reward. DO NOT make the player take another action to get the reward.
